  ...
  return (
    <div className="min-h-screen flex flex-col gradient-winter">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4 space-y-2 pb-24">
        {loading
          ? <div className="flex items-center justify-center py-10"><Loader2 className="w-6 h-6 animate-spin text-[#7CB9E8]" /></div>
          : days.length === 0
            ? (
...
function DayEditor({ day: initial, trip, onClose }) {
  ...
  const addAtv = () => setAtividades((prev) => [...prev, { id: `_new_${Date.now()}_${prev.length}`, _new: true, horario: "", titulo: "", descricao: "", tipo: "passeio", preco: null, status: "confirmado", endereco: "", ordem: prev.length }]);
...
function Field({ label, children }) {
  return (
    <label className="block">
      <span className="text-xs font-display font-bold text-[#1A3A4A]/80">{label}</span>
